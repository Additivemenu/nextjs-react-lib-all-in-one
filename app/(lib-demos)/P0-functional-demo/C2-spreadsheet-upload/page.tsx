...
import { spreadsheetReducer, initialSpreadsheetState } from "./reducers/spreadsheet-reducer";
...
export default function SpreadsheetUploadPage() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.dismiss();
      toast.success(`File uploaded successfully! ${result.data.length} rows loaded.`);
    } catch (error) {
      toast.dismiss();
  ...
  const handleRangeSelectionChanged = (event: RangeSelectionChangedEvent) => {
    ...
    const startColIndex = allColumns?.findIndex(col => col === range.columns[0]) ?? 0;
    const endColIndex = allColumns?.findIndex(col => col === range.columns[range.columns.length - 1]) ?? 0;
    ...
    const startColLetter = String.fromCharCode(65 + Math.min(startColIndex, endColIndex));
    const endColLetter = String.fromCharCode(65 + Math.max(startColIndex, endColIndex));
    const rangeString = `${startColLetter}${Math.min(startRow, endRow) + 1}:${endColLetter}${Math.max(startRow, endRow) + 1}`;
